...
def train(args, model, tokenizer, all_objects_meta):
    ...
    train_dataset = get_task2_dataset(args, tokenizer, all_objects_meta, eval=False)
...
def evaluate(args, model, tokenizer, all_objects_meta):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating", colour='blue', leave=False):
        ...
        with torch.no_grad():
            s_pred_objects, s_true_objects, s_correct_objects, disambiguation_true_items, disambiguation_total_items, disambiguation_pred_list = model.module.evaluate(enc_input, enc_attention_mask, boxes, misc, disambiguation_labels)
    ...
    with open('./disambiguation_labels.json', 'w') as f_out:
...
def main():
    ...
    if args.output_dir:  # 获取 TimeString Dir
        args.output_dir = join(args.output_dir, datetime.now().strftime("%m%d%H%M")+'_vlbert_task2')
    ...
    # 展开训练
    global_step = train(args, model, tokenizer, all_objects_meta)
