...
def read_turn(history_turns, curr_scene, curr_turn_id, all_id_list, all_bbox_list, all_prefab_list, scene_list, all_imgsize_list, agent, file_path, end_flag, history_num):
    ...
    temp_obj_id = get_dict_value(curr_turn, f'{agent}transcript_annotated.act_attributes.objects', None)
    ...
    act_slot_obj = ''
    if 'train' in file_path or ('train' not in file_path and end_flag == True and agent == 'system_'):
        if slot_values != []:
    ...
    if 'train' not in file_path and end_flag == False and agent == 'system_':
...
def main(split):
    ...
    file_path = f'simmc2.1_{split}.tsv'
    if os.path.exists(file_path): os.remove(file_path)
    if os.path.exists(f'{file_path}.index'): os.remove(f'{file_path}.index')

    with open(f'../../simmcdata/simmc2.1_dials_dstc11_{split}.json', 'r') as datafile:
    ...
    agent_list, last_turn = ['system_'], True
